...
import datlt.dtos.RoomErrorObject;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if(check) url = SUCCESS;
                ...
                request.setAttribute("ERRO", "Can not add new");
        } catch (Exception e) {
            if(e.getMessage().contains("duplicate"))
            {
                RoomErrorObject error  = new RoomErrorObject();
                error.setRoomIDError("IDRoom was existed or used to be used before. Check delete list ");
                request.setAttribute("INVALID",error);
                url = INVALID;
            } else
            log("Erorr at Add Room"+e.getMessage());
        } finally {
